...
Аналитик данных с сильной статистической базой: довожу задачу от сырых данных до интерпретируемого решения. Уверенно работаю с SQL и Python, делаю продуктовую аналитику (воронки, retention, когорты, A/B) и прогнозные модели (регрессия, временные ряды, Монте-Карло).
Ценю чистый код, воспроизводимые ноутбуки и проекты на реальных данных.
...
Data analyst with a strong statistical foundation — I take a problem from raw data to an interpretable decision. Confident with SQL and Python; I do product analytics (funnels, retention, cohorts, A/B) and forecasting (regression, time series, Monte-Carlo).
I value clean code, reproducible notebooks, and projects built on real data.
